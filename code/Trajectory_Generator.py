...
def TrajectoryGenerator(Tse_initial, Tsc_initial, Tsc_goal, Tce_grasp, Tce_standoff, k):
    ...
    N = N_driving
    trajectory_1 = np.asarray(core.ScrewTrajectory(
        Tse_initial, Tse_standoff, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_1, N, gripper_state)

    # From standoff position to grasp position:
    N = N_picking
    trajectory_2 = np.asarray(core.ScrewTrajectory(
        Tse_standoff, Tse_grasp, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_2, N, gripper_state)

    # Close the gripper:
    N = N_picking
    gripper_state = 1
    trajectory_3 = np.asarray(core.ScrewTrajectory(
        Tse_grasp, Tse_grasp, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_3, N, gripper_state)

    # From grasp position to standoff position:
    N = N_picking
    trajectory_4 = np.asarray(core.ScrewTrajectory(
        Tse_grasp, Tse_standoff, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_4, N, gripper_state)

    # From standoff position to goal position:
    N = N_picking
    trajectory_5 = np.asarray(core.ScrewTrajectory(
        Tse_standoff, Tse_goal, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_5, N, gripper_state)

    # From goal position to final position:
    N = N_picking
    trajectory_6 = np.asarray(core.ScrewTrajectory(
        Tse_goal, Tse_final, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_6, N, gripper_state)

    # open the gripper:
    N = N_picking
    gripper_state = 0
    trajectory_7 = np.asarray(core.ScrewTrajectory(
        Tse_final, Tse_final, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_7, N, gripper_state)

    # From final position to standoff position:
    N = N_picking
    trajectory_8 = np.asarray(core.ScrewTrajectory(
        Tse_final, Tse_goal, T_tot, N, method))
    trajectory_list = get_list_from_matric(
        trajectory_list, trajectory_8, N, gripper_state)
