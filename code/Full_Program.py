...
def main():
    ...
    for i in range(1, iteration-1):
        ...
        Tsb = np.array([[cos(current_config[0]), -sin(current_config[0]), 0, current_config[1]],
                        [sin(current_config[0]),  cos(
                            current_config[0]), 0, current_config[2]],
                        [0,                       0, 1,            0.0963],
                        [0,                       0, 0,                 1]])
        ...
        Xd = np.array([[trajectory[i][0], trajectory[i][1], trajectory[i][2],  trajectory[i][9]],
                       [trajectory[i][3], trajectory[i][4],
                           trajectory[i][5], trajectory[i][10]],
                       [trajectory[i][6], trajectory[i][7],
                           trajectory[i][8], trajectory[i][11]],
                       [0,                0,                0,                1]])
        print("Xd: ", Xd)

        Xd_next = np.array([[trajectory[i+1][0], trajectory[i+1][1], trajectory[i+1][2],  trajectory[i+1][9]],
                            [trajectory[i+1][3], trajectory[i+1][4],
                                trajectory[i+1][5], trajectory[i+1][10]],
                            [trajectory[i+1][6], trajectory[i+1][7],
                                trajectory[i+1][8], trajectory[i+1][11]],
                            [0,                  0,                  0,                  1]])
        print("Xd_next: ", Xd_next)

        # Calculate the control law:
        V, controls, Xerr, error_integral = FeedbackControl(
            X, Xd, Xd_next, Kp, Ki, delta_t, current_config, error_integral)
        controls_flipped = np.concatenate(
            (controls[4:9], controls[:4]), axis=None)
